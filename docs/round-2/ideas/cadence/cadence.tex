% Build: latexmk -pdf -interaction=nonstopmode -halt-on-error cadence.tex
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Cadence: mathematics with certified computation}
\fancyhead[R]{\small\sffamily Round-two proposal}
\fancyfoot[C]{\thepage}
\hypersetup{pdftitle={Cadence: A Mathematical Language of Certified Transformations},pdfauthor={ChatGPT, prepared for Vladimir Reshetnikov},pdfsubject={Second-round proof-language proposal; Lean, Leant, verified computer algebra}}
...
\vspace{1.05in}
{\sffamily\bfseries\fontsize{37}{42}\selectfont Cadence\par}
\vspace{.20in}
...
In \Cadence, every computational result states its semantic relation to the input:
...
Cadence block was elaborated by Lean. The separate arithmetic companion was run;
...
\Cadence\ is a working name for that interface. Its central object is a
...
The default Cadence publication profile should not rely on undisclosed native
...
source-linked elaboration metadata\cite{leanelab}. Cadence's contribution is not
...
Cadence still needs Lean-checked implementations or a justified interoperability
mechanism.
...
An Cadence source can retain exactly these choices:
\begin{lstlisting}
case n < j:
...
elements of \(M\). Cadence should perform the kernel calculation in \(\Z\), then
...
no-term outcomes\cite{lengine}. These are useful architectural assets. Cadence
...
Cadence's adapter should retain four distinct states:
\begin{lstlisting}
...
\cite{leanelab}. Cadence should work with that machinery rather than replace it with
...
rechecking and challenge-comparison workflows\cite{leanvalidation}. Cadence should
...
replaying that text or retaining its proof term\cite{synthesis,review}. Cadence
...
For Cadence, refining a certified interval for the same root is ordinarily an
...
methods, object interfaces, CAS checkers, and synthesis services; and the Cadence
...
an Cadence parser, a Lean adapter, a verified compiler, a full CAS, or a general
...
Cadence makes the decisive distinctions part of the task: equality or implication,
...
The archive contains \code{cadence.tex}, \code{cadence.pdf},
