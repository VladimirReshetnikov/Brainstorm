% Build: latexmk -pdf -interaction=nonstopmode -halt-on-error vantage.tex
...
\newcommand{\Vantage}{\textsc{Vantage}}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily Vantage: objects, views, and certified computation}
\fancyhead[R]{\small\sffamily Second-round proposal}
\fancyfoot[C]{\small\thepage}
\renewcommand{\headrulewidth}{.3pt}
\hypersetup{pdftitle={Vantage: Mathematical Objects, Explicit Views, and Certified Computation},
...
\vspace{19mm}
{\sffamily\bfseries\fontsize{38}{43}\selectfont\color{ink}Vantage\par}
\vspace{5mm}
...
This article proposes \Vantage, an object-and-view layer whose primary operation is
...
\Vantage\ adopts those ideas. Its proposed increment is to give them a more concrete
...
\Vantage\ does not make CAS simplification part of Lean's definitional equality.
...
showing that the value actually used is its result is another. \Vantage's strict
...
For \Vantage, the first integration target is term assembly across fixed interfaces.
...
A published \Vantage\ proof bundle is specified to contain the source document, an expanded Lean companion,
...
Lean already separates elaboration from its small logical checker; \Vantage\ uses
...
refinement framework. \Vantage\ proposes a language-facing organization around
...
\Vantage\ keeps the first round's conservative logical foundation and makes the
